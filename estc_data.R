...
dplyr::rename(id = 1) %>% mutate(bernard = as.integer(bernard)) %>% mutate(is_organization = as.character(is_organization))

clean <- read.csv("csv/spectator_clean.csv", stringsAsFactors = FALSE, sep = ";")

is_pure <- read.csv("csv/pure_spectator.csv", stringsAsFactors = FALSE, sep = ";") %>% dplyr::rename(id = 1)
